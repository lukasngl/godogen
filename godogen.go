...
func (file *fileVisitor) visitFuncDecl(funcdecl *ast.FuncDecl) []StepFunc {
	...
	slog.Info(
		...
		"steps", len(stepFunc.Steps),
		"hooks", len(stepFunc.Hooks),
	)
...
func (stepFunc *StepFunc) validate(fset *token.FileSet) {
	...
	} else if len(stepFunc.Hooks) > 0 {
		stepFunc.ValidationErrors = validateHookFunc(fset, stepFunc.Node)
...
func validateHookFunc(
	...
	if params != nil {
		paramTypes = params.List
	}

	switch len(paramTypes) {
	case 2:
		// Two return values: should be (context.Context, error).
		paramTypeA := exprToString(fset, paramTypes[0].Type)
		paramTypeB := exprToString(fset, paramTypes[1].Type)
		if paramTypeA != "context.Context" || paramTypeB != "*godog.ScenarioContext" {
			errs = append(errs, Error{
				Message: "function should have (context.Context, *godog.ScenarioContext) as params",
				Node:    params,
			})
		}
	default:
		// Not exactly two return values.
		errs = append(errs, Error{
			Message: "function should have (context.Context, *godog.ScenarioContext) as params",
			Node:    params,
		})
	...
	case 2:
		...
		if resultTypeA != "context.Context" || resultTypeB != "error" {
			errs = append(errs, Error{
				Message: "function should return (context.Context, error)",
				Node:    results,
			})
		}
	default:
		// Not exactly two return values.
		errs = append(errs, Error{
			Message: "function should return exactly (context.Context, error)",
			Node:    results,
		})
	}
