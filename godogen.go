...
type (
	...
	Error struct {
		...
		// Pos is start of the part of the source code in which the error occurred.
		Pos token.Pos
		// Pos is end of the part of the source code in which the error occurred.
		End token.Pos
	}
)
...
func (visitor *fileVisitor) VisitComment(funcdecl *ast.FuncDecl, comment *ast.Comment) []Step {
	...
	case "before", "after", "step", "given", "when", "then":
		...
		if directive != "before" && directive != "after" {
			step.ValidationErrors = visitor.validateStep(funcdecl, step)
		}
